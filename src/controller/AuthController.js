...
class AuthController {
  ...
  static async passwordlessLogin(req, res) {
    const res_obj = await AuthService.passwordlessLogin(req.body);
    res.status(201).send(res_obj);
  }
  static async verfiyOtp(req, res) {
    const res_obj = await AuthService.verfiyOtp(req.body);
    res.status(200).send(res_obj);
  }
